...
build_desura() {
	...
	cd build
	cmake .. -DCMAKE_INSTALL_PREFIX=$PREFIX -DBUILD_CEF=OFF -DWITH_GTEST=OFF -DOFFICIAL_BUILD=$OFFICIAL_BUILD -DFORCE_BUNDLED_WXGTK=$OFFICIAL_BUILD -DFORCE_BUNDLED_BOOST=$OFFICIAL_BUILD || exit 1
	NUM_PROC=`nproc`
...
pack() {
	...
	cd build_package
	cmake .. -DPACKAGE_TYPE=$PACKAGE -DINSTALL_DESKTOP_FILE=ON -DCMAKE_INSTALL_PREFIX="/opt/desura" -DDESKTOP_EXE="/opt/desura/desura" -DDESKTOP_ICON="/opt/desura/desura.png" -DOFFICIAL_BUILD=$OFFICIAL_BUILD -DFORCE_BUNDLED_WXGTK=$OFFICIAL_BUILD -DFORCE_BUNDLED_BOOST=$OFFICIAL_BUILD || exit 1
	NUM_PROC=`nproc`
	echo "${NUM_PROC} processors detected"
