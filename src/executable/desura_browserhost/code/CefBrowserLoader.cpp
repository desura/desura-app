...
CVar gc_cef_timeout("gc_cef_timeout", "50", 0, &RestartTimerCB);
#endif
...
bool InitWebControl(const char* hostName, const char* userAgent)
{
	...
	ua += " " + gcString(userAgent);

	UTIL::FS::Path path(UTIL::OS::getTempInternetPath(gcWString(L"desura-{0}", hostName)), L"", false);
